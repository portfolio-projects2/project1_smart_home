...
@Autowired
    ...
    if(System.getenv("dataSource").equals("docker")){
        dataSourceURL= coreSecrets.getUrl2Docker();
    }else {dataSourceURL=coreSecrets.getUrl2(); }
...
@Override
    ...
    ResultSet resultSet= preparedStatement.executeQuery();
    while (resultSet.next()){
        Device device = new afgangsProjekt.automation.database2.DatabaseDevice(
